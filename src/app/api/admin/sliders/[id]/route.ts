...
export async function GET(
  ...
  try {
    const result = await query('SELECT * FROM "Slider" WHERE "id" = $1', [parseInt(params.id)])
...
export async function PATCH(
  ...
  try {
    ...
    const result = await query(
      `UPDATE "Slider" SET ${fields.join(', ')} WHERE "id" = $${paramIndex} RETURNING *`,
      values
    )
...
export async function DELETE(
  ...
  try {
    await query('DELETE FROM "Slider" WHERE "id" = $1', [parseInt(params.id)])
